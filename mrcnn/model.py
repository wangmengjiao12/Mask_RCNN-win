...
def load_image_gt(dataset, config, image_id, augmentation=None):
    """加载并返回图像的 ground truth (image, mask, bounding boxes).

    augmentation: Optional. An imgaug (https://github.com/aleju/imgaug) augmentation.
        例如, 使用 imgaug.augmenters.Fliplr(0.5) 将 50%时的 images 左/右翻转.

    Returns:
    image: [height, width, 3]
    shape: 调整大小和裁剪之前图像的原始shape.
    class_ids: [instance_count] Integer class IDs
    bbox: [instance_count, (y1, x1, y2, x2)]
    mask: [height, width, instance_count]. 高度和宽度是图像的高度和宽度，
    在 use_mini_mask为True情况下，它们是在MINI_MASK_SHAPE中定义的.
    """
    # 加载 image and mask
    ...
    if augmentation:
        ...
        # 可用于安全增强mask的有一些产品具有使他们不安全的设置(如Affine)，
        # 请在mask上测试您的Augmentation
        ...
        # 在augmentation前存储shapes以用于比较
        image_shape = image.shape
        mask_shape = mask.shape
        # 使 augmenters 确定被用于 images and masks
        det = augmentation.to_deterministic()
        image = det.augment_image(image)
        # 将 mask 改成 np.uint8 因为 imgaug 不支持np.bool
    ...
    # 注意，如果裁剪出相应的掩码，则某些框会全为0，这时将他们过滤掉
    _idx = np.sum(mask, axis=(0, 1)) > 0
    mask = mask[:, :, _idx]
    class_ids = class_ids[_idx]
    # Bounding boxes. 注意某些框全为0
    # 若相应的mask被裁剪掉.
    # bbox: [num_instances, (y1, x1, y2, x2)]
    bbox = utils.extract_bboxes(mask)

    # 活动类不同的dataset有不同的类，请跟踪此图像的数据集中支持的类
    active_class_ids = np.zeros([dataset.num_classes], dtype=np.int32)
    source_class_ids = dataset.source_class_ids[dataset.image_info[image_id]["source"]]
    active_class_ids[source_class_ids] = 1

    # 将 masks 调整到较小的尺寸以减少 memory usage
    if config.USE_MINI_MASK:
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, config):
    """生成用于阶段2中classifier and mask heads 的目标.
        正常训练不适用此功能，但对于不使用RPN head 来调试或训练Mask RCNN heads 十分有用.

    Inputs:
    rpn_rois: [N, (y1, x1, y2, x2)] proposal boxes.
    gt_class_ids: [instance count] Integer class IDs
    gt_boxes: [instance count, (y1, x1, y2, x2)]
    gt_masks: [height, width, instance count] Ground truth masks. 可以是full-size/mini-masks.
    ...
    masks: [TRAIN_ROIS_PER_IMAGE, height, width, NUM_CLASSES).
           类特定的masks 裁剪到bbox 边界并调整为神经网络输出大小.
    """
    ...
    # 通常将GT Boxes 加入到ROI 中，但是我们不这样做，因为根据XinLei Chen's paper,这没有帮助.

    # 修改gt_boxes and gt_masks 部分中的空白填充
    ...
    # 计算ROIs and ground truth boxes 的区域.
    rpn_roi_area = (rpn_rois[:, 2] - rpn_rois[:, 0]) * \
        (rpn_rois[:, 3] - rpn_rois[:, 1])
    gt_box_area = (gt_boxes[:, 2] - gt_boxes[:, 0]) * \
        (gt_boxes[:, 3] - gt_boxes[:, 1])

    # 计算重叠 [rpn_rois, gt_boxes]
    ...
    # Positive ROIs 是 IoU with a GT box >= 0.5 的ROIs .
    fg_ids = np.where(rpn_roi_iou_max > 0.5)[0]

    # Negative ROIs 是 max IoU 为0.1-0.5 (hard example mining)
    # TODO: To hard example mine or not to hard example mine, that's the question
    # bg_ids = np.where((rpn_roi_iou_max >= 0.1) & (rpn_roi_iou_max < 0.5))[0]
    bg_ids = np.where(rpn_roi_iou_max < 0.5)[0]

    # Subsample ROIs. 目标是 33% foreground.
    # FG
    ...
    if remaining > 0:
        # 我们没有足够的samples to 保证需求平衡
        # 减少需求并填写其余部分. 这与 Mask RCNN paper 有所不同.
    ...
    # 重置分配给 BG ROIs 的gt boxes .
    rpn_roi_gt_boxes[keep_bg_ids, :] = 0
    rpn_roi_gt_class_ids[keep_bg_ids] = 0

    # 对每个保留的 ROI, 分配 class_id, 对于 FG ROIs 应添加 bbox 优化.
    rois = rpn_rois[keep]
    ...
    for i in pos_ids:
        ...
        if config.USE_MINI_MASK:
            # Create a mask placeholder(占位符), the size of the image
...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
    """给定 anchors and GT boxes, 计算 overlaps and 确定并优化 positive anchors 和 deltas
       来匹配其对应的GT boxes.
    ...
    Returns:
    rpn_match: [N] (int32) anchors and GT boxes 间的匹配.
               1 = positive anchor, -1 = negative anchor, 0 = neutral
    rpn_bbox: [N, (dy, dx, log(dh), log(dw))] Anchor bbox deltas.
    """
    # RPN Match: 1 = positive anchor, -1 = negative anchor, 0 = neutral
    rpn_match = np.zeros([anchors.shape[0]], dtype=np.int32)
    # RPN bounding boxes: [每个image 的最大anchors , (dy, dx, log(dh), log(dw))]
    rpn_bbox = np.zeros((config.RPN_TRAIN_ANCHORS_PER_IMAGE, 4))

    # Handle COCO crowds
    # COCO中的crowd box是围绕多个实例的边界框，将其排除在训练之外. crowd box被赋予了 negative的 class ID
    crowd_ix = np.where(gt_class_ids < 0)[0]
    if crowd_ix.shape[0] > 0:
        # 从ground truth class IDs and boxes 过滤出 crowds
        non_crowd_ix = np.where(gt_class_ids > 0)[0]
        crowd_boxes = gt_boxes[crowd_ix]
        gt_class_ids = gt_class_ids[non_crowd_ix]
        gt_boxes = gt_boxes[non_crowd_ix]
        # 计算 overlaps with crowd boxes [anchors, crowds]
        crowd_overlaps = utils.compute_overlaps(anchors, crowd_boxes)
        crowd_iou_max = np.amax(crowd_overlaps, axis=1)
        no_crowd_bool = (crowd_iou_max < 0.001)
    else:
        # 所有 anchors 都不会与crowd 相交
        no_crowd_bool = np.ones([anchors.shape[0]], dtype=bool)

    # 计算 overlaps [num_anchors, num_gt_boxes]
    overlaps = utils.compute_overlaps(anchors, gt_boxes)

    # 将 anchors 匹配到 GT Boxes
    # 若anchor 与GT box 的IoU >= 0.7 ，则 positive.
    # 若anchor 与GT box 的IoU <= 0.3 ，则 negative.
    # Neutral anchors 是不符合上述条件的锚点, 它们不影响损失函数
    # 但是不要让所有的GT box 无法匹配(少，但是会发生) (即使设置最大IoU < 0.3).
    #
    # 1. 先设置negative anchors. 如果GT box 与之匹配，则其接下来会被overwritten.
    #    跳过在crowd 区域的box.
    anchor_iou_argmax = np.argmax(overlaps, axis=1)
    anchor_iou_max = overlaps[np.arange(overlaps.shape[0]), anchor_iou_argmax]
    rpn_match[(anchor_iou_max < 0.3) & (no_crowd_bool)] = -1
    # 2. 为每个GT box 设置一个anchor(无论其IoU 值如何).如果多个anchor 具有相同的IoU，则将其全部匹配
    gt_iou_argmax = np.argwhere(overlaps == np.max(overlaps, axis=0))[:, 0]
    rpn_match[gt_iou_argmax] = 1
    # 3. 将重叠度高的anchors 设置为positive.
    rpn_match[anchor_iou_max >= 0.7] = 1

    # 用Subsample 来平衡positive and negative anchors，不要让positives anchors 超过总anchors 的一半
    ...
    # 对positive anchors, 计算将其变换为相应的GT boxes 所需的shift and scale.
    ids = np.where(rpn_match == 1)[0]
    ix = 0  # rpn_bbox 的索引
    # TODO: use box_refinement() rather than duplicating the code here
    for i, a in zip(ids, anchors[ids]):
        # 最近的gt box (它的 IoU < 0.7)
        gt = gt_boxes[anchor_iou_argmax[i]]

        # 将坐标转化为 中心加width/height.
        # GT Box
        gt_h = gt[2] - gt[0]
        gt_w = gt[3] - gt[1]
...
def generate_random_rois(image_shape, count, gt_class_ids, gt_boxes):
    """类似于生成region proposal network 来生成ROI proposals.
    ...
    for i in range(gt_boxes.shape[0]):
        ...
        # 为避免生成面积为0的boxes ，将需要的东西进行加倍并过滤多余的东西，
        # 若得到的有效boxes 少于所需，则循环并重试.
        while True:
        ...
        # 在轴上排序来确保 x1 <= x2 and y1 <= y2 然后重塑为x1, y1, x2, y2 顺序
        x1, x2 = np.split(np.sort(x1x2, axis=1), 2, axis=1)
        y1, y2 = np.split(np.sort(y1y2, axis=1), 2, axis=1)
        box_rois = np.hstack([y1, x1, y2, x2])
        rois[rois_per_box * i:rois_per_box * (i + 1)] = box_rois

    # 在image 的任何位置生成随机的ROI (占总数的10%)
...
class DataGenerator(KU.Sequence):
    """一个可迭代的函数,它返回图像和相应的 class ids, bounding box deltas, and masks
        它继承自 keras.utils.Sequence 以避免在 multiprocessing=True时出现数据冗余.

        dataset: The Dataset object to pick data from
        config: The model config object
        shuffle: If True, 对每个 epoch 前对样本进行乱序
        augmentation: Optional. An imgaug (https://github.com/aleju/imgaug) augmentation.
            For example, passing imgaug.augmenters.Fliplr(0.5) flips images
            right/left 50% of the time.
        random_rois: If > 0 then 生成用于训练网络分类器和mask heads的 提议，
                     在没有RPN的情况下对训练mask rcnn的部件很有用.
        detection_targets: If True, 生成检测目标(class IDs, bbox deltas, and masks).
            常用于调试或可视化，因为在训练中检测目标由DetectionTargetLayer生成.

        返回一个Python iterable. 对其调用 __getitem__() 时,迭代器返回两个列表：inputs and outputs.
        列表内容根据收到的参数而有所不同:
        inputs list:
        - images: [batch, H, W, C]
        - image_meta: [batch, (meta data)] Image details. See compose_image_meta()
        ...
        - gt_masks: [batch, height, width, MAX_GT_INSTANCES]. 高度和宽度是图像的高度和宽度，
                    在use_mini_mask 为 True 的情况下，它们是在MINI_MASK_SHAPE中定义的.

        outputs list: 通常在训练中是空的. 但若detection_targets = True
            则输出列表包含 class_ids, bbox deltas, and masks.
        """
    ...
    def __getitem__(self, idx):
        ...
        while b < self.batch_size:
            ...
            # Mask R-CNN 目标
            if self.random_rois:
                # 随机生成ROI，其中90%在GT boxes 周围，10%随机.
            ...
            # 如果 instances 的数目超过数组的大小，需要从中进行 sub-sample.
...
class MaskRCNN(object):
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,
                          image_shape, window):
        """将一幅图像的检测从神经网络输出的格式重新格式化为适合在其余应用中使用的格式.
        ...
        # 在调整大小之前，将调整大小后的图像中的规范化坐标转换为原始图像中的像素坐标
        ...
        # 将 mask 调整为原始图像大小并设置边界阈值.
        full_masks = []
        for i in range(N):
            # 将神经网络 mask 转换为全尺寸 mask
    ...
    def detect(self, images, verbose=0):
        ...
        images: images 列表, 可能大小不一.

        返回一个字典列表, 每个图像一个字典. 字典包含:
        rois: [N, (y1, x1, y2, x2)] 检测边界框
        class_ids: [N] 整型 class IDs
        scores: [N] class IDs 的浮动概率得分数
        masks: [H, W, N] 实例的二进制 masks
        """
        assert self.mode == "inference", "Create model in inference mode."
        assert len(
            images) == self.config.BATCH_SIZE, "len(images) must be equal to BATCH_SIZE"

        # 若verbose不是0，则打印images的信息
        if verbose:
    ...
    def run_graph(self, images, outputs, image_metas=None):
        ...
        image_metas: 如果提供image，则假定image已经经过处理成型 (i.e. resized, padded, and normalized)

        outputs: 要计算的元组列表 (name, tensor) . tensors象征着 TensorFlow tensors，因为易跟踪得到这个名字.

        返回结果是有序字典. Keys ：输入中接收到的名称， value：Numpy数组.
        """
...
def compose_image_meta(image_id, original_image_shape, image_shape,
                       window, scale, active_class_ids):
    """获取 image的属性，并将其放到1D array.

    image_id: An int ID of the image. Useful for debugging.
    original_image_shape: [H, W, C] before resizing or padding.
    image_shape: [H, W, C] after resizing and padding
    window: (y1, x1, y2, x2) in pixels. 真实图像所在区域 (不包括填充)
    scale: The scaling factor applied to the original image (float32)
    active_class_ids: 图像来源的数据集中可用的class_id列表。
                      如果对来自多个数据集的图像进行训练，而并非所有数据集中都没有所有类别，
                      则很有用.
    """
    meta = np.array(
        [image_id] +                  # size=1
